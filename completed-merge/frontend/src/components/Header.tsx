...
export const Header: React.FC<HeaderProps> = ({ issues, notifications, setNotifications, onSelectIssue, onOpenModal }) => {
  ...
  const getNotifIcon = (type: string) => {
    switch(type) {
      ...
      case 'comment': return <svg fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="mp-icon-small"><path strokeLinecap="round" strokeLinejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m0 12.75h7.5m-7.5 3H12M10.5 2.25H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" /></svg>;
      default: return '';
    }
  };
  ...
  return (
    <header className="mp-global-header">
      <div className="mp-header-logo-section" onClick={() => onSelectIssue(null)}>
      ...
      <div ref={searchRef} className="mp-search-wrapper">
        <div className="mp-search-input-container">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="mp-search-icon">
          ...
          {isSearchOpen && (
            <div className="mp-search-dropdown">
              <div className="mp-search-dropdown-header">
                {filteredSearch.length} results
              </div>
              <div className="mp-search-results-list">
                {filteredSearch.length === 0 ? (
                  <div className="mp-search-no-results">No matching issues found</div>
                ) : (
      ...
      <div className="mp-header-actions">
        <button onClick={onOpenModal} id="new-issue-btn" type="button" className="mp-primary-btn">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" className="mp-icon-small"><path strokeLinecap="round" strokeLinejoin="round" d="M12 4.5v15m7.5-7.5h-15" /></svg>
          New Issue
        </button>
        
        {/* BELL NOTIFICATIONS */}
        <div ref={notifRef} className="mp-bell-wrapper">
          <button onClick={() => setIsNotifOpen(!isNotifOpen)} id="bell-btn" type="button" className="mp-bell-trigger">
            ...
            {unreadCount > 0 && <span id="bell-badge" className="mp-bell-badge">{unreadCount}</span>}
          </button>
          ...
          {isNotifOpen && (
            <div id="notifications-dropdown" className="mp-notifications-dropdown">
              <div className="mp-notif-dropdown-header">
                ...
                <button onClick={() => setNotifications(notifications.map(n => ({...n, unread: false})))} id="mark-all-read-btn" type="button" className="mp-mark-read-btn">Mark all as read</button>
              </div>
              <div id="notifications-list" className="mp-notifications-list">
                {notifications.length === 0 ? (
                  <div className="mp-notif-empty-state">You're all caught up!</div>
                ) : (
                  notifications.map(n => (
                    <div key={n.id} onClick={() => { onSelectIssue(n.targetId); setIsNotifOpen(false); }} className="mp-notif-card">
        ...
        <button className="mp-user-avatar-btn">AJ</button>
      </div>
    </header>
  );
};